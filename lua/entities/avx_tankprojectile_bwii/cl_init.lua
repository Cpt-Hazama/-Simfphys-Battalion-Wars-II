...
function ENT:doFX( newpos, oldpos )
	...
	if self.EffectType == 0 then -- Normal
		for i = 1, Len, 25 do
			...
			if particle then
				...
				particle:SetStartSize((math.Rand(6,12) / 20) *self:GetSize() *1.65)
				particle:SetEndSize((math.Rand(20,30) / 20) *self:GetSize() *1.65)
				particle:SetRoll(math.Rand(-1,1))
				-- particle:SetColor(130,130,130)
				particle:SetColor(150,137,104)
				particle:SetCollide(false)
			end
		
			local particle = self.emitter:Add( mat, pos ) // Glow
			if particle then
				particle:SetVelocity( -self:GetForward() * 300 + self:GetVelocity())
				particle:SetDieTime( 0.1 )
				particle:SetAirResistance( 0 ) 
				particle:SetStartAlpha( 255 )
				particle:SetStartSize( self:GetSize() *2.2578 )
	...
	elseif self.EffectType == 1 then -- Legion
		for i = 1, Len, 25 do
			local pos = oldpos + Dir * i
			...
			if particle then
				...
				particle:SetAirResistance( 0 ) 
				particle:SetStartAlpha( 255 )
				particle:SetStartSize( self:GetSize() *1.3 )
				particle:SetEndSize( 0 )
				particle:SetRoll( math.Rand(-1,1) )
	...
	elseif self.EffectType == 2 then -- Solar
		for i = 1, Len, 25 do
			local pos = oldpos + Dir * i
