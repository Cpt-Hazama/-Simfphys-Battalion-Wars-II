	-- Variables --
local class = "bwii_iltank"
local icon = "entities/bwii_iltank.png"
...
local fireSound = "cpthazama/bwii/ironlegion/WU_Baz_Fire.wav"
...
local secondSound = "cpthazama/bwii/ironlegion/WU_HMG_Fire.wav"
...
local ppSideTurretYaw = "lgun_yaw"
local ppSideTurretPitch = "lgun_pitch"
...
function simfphys.weapon:ControlTrackSounds( vehicle, wheelslocked ) 
	...
	local fastenuf = speed > 0 and not wheelslocked and self:IsOnGround( vehicle )
	
	if fastenuf ~= vehicle.fastenuf then
		vehicle.fastenuf = fastenuf
		
		if fastenuf then
			vehicle.track_snd = CreateSound( vehicle, "cpthazama/bwii/ironlegion/VU_HTank_Eng2.wav" )
...
function simfphys.weapon:DoWheelSpin( vehicle )
	...
	vehicle:SetPoseParameter("lWheels", -spin_r)
	vehicle:SetPoseParameter("rWheels", -spin_l )
