	-- Variables --
local class = "bwii_xantiair"
local icon = "entities/bwii_xantiair.png"
...
local fireSound = "cpthazama/bwii/xylvania/WX_Flak_Fire.wav"
...
local function primary_fire(ply,vehicle,shootOrigin,shootDirection,forcelock)
	...
	local Pos = vehicle:GetAttachment(vehicle.Tube).Pos
	ent:SetPos(Pos)
	...
	if IsValid(vehicle.LockTarget) then
		local Target = vehicle.LockTarget
		if string.find(Target:GetClass(),"lunasflightschool") && !string.find(Target:GetClass(),"_bwii_missile") then
