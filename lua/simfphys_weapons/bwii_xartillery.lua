	-- Variables --
local class = "bwii_xartillery"
local icon = "entities/bwii_xartillery.png"
local name = "Artillery"
local TEAM = TEAM_XYLVANIA
local driverFPPos = Vector(-268,-5,0)
...
local secondSound = "cpthazama/bwii/xylvania/machinegun_fire.wav"
...
function simfphys.weapon:ControlTrackSounds( vehicle, wheelslocked ) 
	...
	if fastenuf ~= vehicle.fastenuf then
		...
		if fastenuf then
			vehicle.track_snd = CreateSound( vehicle, "cpthazama/bwii/xylvania/lighttank_eng1.wav" )
...
function simfphys.weapon:DoWheelSpin( vehicle )
	...
	vehicle:SetPoseParameter("lwheela", -spin_l)
	vehicle:SetPoseParameter("lwheelb", -spin_l)
	vehicle:SetPoseParameter("lwheelc", -spin_l)
	vehicle:SetPoseParameter("lwheeld", -spin_l)
	vehicle:SetPoseParameter("rwheela", -spin_r)
	vehicle:SetPoseParameter("rwheelb", -spin_r)
	vehicle:SetPoseParameter("rwheelc", -spin_r)
	vehicle:SetPoseParameter("rwheeld", -spin_r)
