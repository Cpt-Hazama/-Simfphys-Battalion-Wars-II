	-- Variables --
local class = "bwii_threcon"
local icon = "entities/bwii_threcon.png"
local name = "Heavy Recon"
local TEAM = TEAM_TUNDRA
local driverFPPos = Vector(0,0,0)
local driverTPPos = Vector(0,0,0)
local driverFollowerAttachment = true
local crosshairDirection = Vector(0,90,0)
local gunnerFPPos = Vector(0,0,25)
local gunnerTPPos = Vector(0,0,0)
local gunnerBFPPos = Vector(0,0,2)
local gunnerBTPPos = Vector(0,0,0)
...
local ppTurretYaw = "cturret_yaw"
local ppTurretPitch = "cturret_pitch"
...
local ppSideTurretYaw = "rturret_yaw"
local ppSideTurretPitch = "rturret_pitch"
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCrosshair( vehicle.pSeat[2] , { Attachment = "muzzleTurret", Type = 1 } )
	simfphys.RegisterCamera( vehicle.pSeat[2], gunnerBFPPos, gunnerBTPPos, false, nil )
...
local function primary_fire(ply,vehicle,shootOrigin,shootDirection)
	vehicle:EmitSound("cpthazama/bwii/tundra/WT_HMG_Stop.wav", 95, 100)
	local projectile = {}
...
function simfphys.weapon:ControlTrackSounds( vehicle, wheelslocked ) 
	...
	if fastenuf ~= vehicle.fastenuf then
		...
		if fastenuf then
			vehicle.track_snd = CreateSound( vehicle, "cpthazama/bwii/tundra/VW_LTrans_Eng.wav" )
...
function simfphys.weapon:AimMachinegun( ply, vehicle, pod )	
	...
	vehicle:SetPoseParameter(ppSideTurretYaw, Yaw)
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )
	...
	local Pitch = EyeAngles.r

	vehicle:SetPoseParameter("cturret_yaw", Yaw)
	vehicle:SetPoseParameter("cturret_pitch", Pitch )
end
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i = 1,#tblSuspensionData do
		...
		local Dist = pos:Distance(trace.HitPos) /5
		local inAir = false
		local pp = Dist
...
function simfphys.weapon:DoWheelSpin( vehicle )
	...
	local turn = vehicle:GetVehicleSteer() *10 -- -1 or 1
	local lwheel = 7
	local rwheel = 5
	...
	vehicle:SetPoseParameter("rwheel_turn", turn)
	vehicle:SetPoseParameter("rbwheel", -spin_r)
	vehicle:SetPoseParameter("lwheel", -spin_l )
	vehicle:SetPoseParameter("lwheel_turn", turn )
