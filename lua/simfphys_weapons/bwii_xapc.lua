	-- Variables --
local class = "bwii_xapc"
local icon = "entities/bwii_xapc.png"
local name = "APC"
local TEAM = TEAM_XYLVANIA
local driverFPPos = Vector(0,-5,12)
...
function simfphys.weapon:AI_Aim(enemy,vehicle,Attachment)
	...
	vehicle:SetPoseParameter(ppSideTurretPitch,-math.ApproachAngle(vehicle:GetPoseParameter(ppSideTurretPitch),pitch,AimRate))
	vehicle:SetPoseParameter(ppSideTurretYaw,math.ApproachAngle(vehicle:GetPoseParameter(ppSideTurretYaw),yaw,AimRate))
end
...
function simfphys.weapon:AI_Turret(vehicle,deltapos,enemy)
	...
	-- if fire then
		self:SecondaryAttack(vehicle,vehicle,shootOrigin,Attachment)
	-- end
end
