	-- Variables --
local class = "bwii_srecon"
local icon = "entities/bwii_srecon.png"
local name = "Light Recon"
local TEAM = TEAM_SOLAR
local driverFPPos = Vector(0,-8,6)
...
function simfphys.weapon:ControlTurret( vehicle, deltapos )
	...
	if fire then
		if !vehicle.Loop:IsPlaying() then
			vehicle.Loop:Play()
		end
...
local function primary_fire(ply,vehicle,shootOrigin,shootDirection,Attachment)
	local bullet = {}
	bullet.Num 			= 1
...
function simfphys.weapon:ControlTrackSounds( vehicle, wheelslocked ) 
	...
	if fastenuf ~= vehicle.fastenuf then
		...
		if fastenuf then
			...
			vehicle.track_snd:ChangePitch(200)
