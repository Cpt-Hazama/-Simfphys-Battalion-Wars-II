	-- Variables --
local class = "bwii_aartillery"
local icon = "entities/bwii_aartillery.png"
local name = "Artillery"
local TEAM = TEAM_ANGLO
...
local gunnerTPPos = Vector(0,0,20)
...
local mainForceOnVehicle = 900000
local mainDMG = BWII_DMG_ARTILLERY
local mainForce = 15000
local mainEffectSize = 25
local mainRadius = 500
local mainRadiusDMG = BWII_DMG_ARTILLERY /20

local secondaryAttachment = "lt"
local secondaryCooldown = 0.1
local ppSideTurretYaw = "gun_yaw"
local ppSideTurretPitch = "gun_pitch"
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "lt", Type = 1 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], gunnerFPPos, gunnerTPPos, gunnerFollowAttachment, nil )
	vehicle.pSeat[1]:SetNWBool("BWII_StandTurret",true)
...
function simfphys.weapon:ControlTurret( vehicle, deltapos )
	...
	vehicle.NextChatT = vehicle.NextChatT or CurTime() +1
	vehicle.LastPitch = vehicle.LastPitch or vehicle:GetPoseParameter(ppTurretPitch)
	vehicle.LastAngle = vehicle.LastAngle or vehicle:GetAngles().yaw

	if fire then
		self:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
	end
	if fire2 && CurTime() > vehicle.NextChatT then
		local oPP = vehicle.LastPitch
		local pp = vehicle:GetPoseParameter(ppTurretPitch)
		vehicle.LastPitch = pp
		local oY = vehicle.LastAngle
		local y = vehicle:GetAngles().yaw
		vehicle.LastAngle = y
		local y = vehicle.LastAngle
		ply:ChatPrint("Pitch = " .. tostring(math.Round(pp,1)) .. " | Previous Pitch = " .. tostring(math.Round(oPP,1)))
		ply:ChatPrint("Yaw = " .. tostring(math.Round(y,1)) .. " | Previous Yaw = " .. tostring(math.Round(oY,1)))
		vehicle.NextChatT = CurTime() +0.5
	end
end
...
local function primary_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	projectile.MuzzleVelocity = vehicle.LaunchForce
	
	local proj = AVX.FirePhysProjectile_Return(projectile)
end
...
function simfphys.weapon:ControlMachinegun( vehicle, deltapos )
	...
	if fire then
		...
		if !vehicle.GasEffect then
			vehicle.GasEffect = CreateSound(vehicle,secondSound)
			vehicle.GasEffect:SetSoundLevel(90)
		end
		if CurTime() > vehicle.NextGasTime then
			vehicle.GasEffect:Stop()
			vehicle.GasEffect:Play()
			vehicle.NextGasTime = CurTime() +SoundDuration(secondSound)
		end
	else
		if vehicle.GasEffect then
			vehicle.GasEffect:Stop()
...
function simfphys.weapon:ControlTrackSounds( vehicle, wheelslocked ) 
	...
	local fastenuf = speed > 15 and not wheelslocked and self:IsOnGround( vehicle )
	
	if fastenuf ~= vehicle.fastenuf then
		vehicle.fastenuf = fastenuf
		
		if fastenuf then
			vehicle.track_snd = CreateSound( vehicle, "cpthazama/bwii/angloisles/VA_Artillery_Eng2.wav" )
...
function simfphys.weapon:Think( vehicle )
	...
	self.cooldownT = self.cooldownT or 0
...
function simfphys.weapon:SecondaryAttack( vehicle, ply, shootOrigin, Attachment, ID )
	...
	if !self.fireRate then self.fireRate = 0.4 end
	local change = self.fireRate -0.03
	local cl = math.Clamp(change,0.1,0.4)
	self.fireRate = cl
	...
	self.cooldownT = CurTime() +0.8
	
	self:SetNextSecondaryFire( vehicle, CurTime() + self.fireRate )
end
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )
	...
	if IsValid(vehicle.LockTarget) then
		...
		local pitch = math.AngleDifference(targetang.p,selfang.p)
		local yaw = math.AngleDifference(targetang.y,selfang.y)
		local dist = selfpos:Distance(enemy:GetPos())
		local change = (dist *0.2) /34
		local force = change -1
		vehicle.LaunchForce = math.Clamp(force,20,50)
		print(dist,change)
		vehicle:SetPoseParameter(ppTurretPitch,math.ApproachAngle(vehicle:GetPoseParameter(ppTurretPitch),change,AimRate) +ppTurretPitchAddition)
		vehicle:SetPoseParameter(ppTurretYaw,math.ApproachAngle(vehicle:GetPoseParameter(ppTurretYaw),yaw,AimRate))
	else
		vehicle.LaunchForce = 76
		local AimRate = chasisTurnSpeed
