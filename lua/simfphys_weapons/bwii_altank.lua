	-- Variables --
local class = "bwii_altank"
local icon = "entities/bwii_altank.png"
local name = "Light Tank"
local TEAM = TEAM_ANGLO
local driverFPPos = Vector(-45,-30,0)
...
local function secondary_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	local att = vehicle:GetAttachment(2)
	local effect = EffectData()
	effect:SetStart(att.Pos)
	effect:SetNormal(att.Ang:Forward())
	effect:SetEntity(vehicle)
	effect:SetAttachment(2)
	util.Effect("xltank_acid",effect)
	
	for _,ent in ipairs(ents.FindInSphere(shootOrigin,400)) do -- Acid gun
		if (ent:IsNPC() || (ent:IsPlayer() && ent != ply)) && (shootDirection:Dot(((ent:GetPos() +ent:OBBCenter()) -shootOrigin +shootDirection *1):GetNormalized()) > math.cos(math.rad(20))) then
