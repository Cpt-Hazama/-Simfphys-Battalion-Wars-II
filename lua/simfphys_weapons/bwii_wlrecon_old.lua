	-- Variables --
local class = "bwii_wlrecon_old"
local icon = "entities/bwii_wlrecon_old.png"
local name = "Light Recon"
local TEAM = TEAM_FRONTIER
local driverFPPos = Vector(0,-8,0)
local driverTPPos = Vector(0,0,0)
local driverFollowerAttachment = true
local crosshairDirection = Vector(0,90,0)
local gunnerFPPos = Vector(0,0,4)
...
function simfphys.weapon:Initialize( vehicle )
	...
	vehicle:ManipulateBoneJiggle(11,1)
	vehicle:ManipulateBoneJiggle(12,1)
...
local function primary_fire(ply,vehicle,shootOrigin,shootDirection)
	vehicle:EmitSound("cpthazama/bwii/westernfrontier/WW_HMG_Fire_2.wav", 95, 100)
	local projectile = {}
...
function simfphys.weapon:ControlTrackSounds( vehicle, wheelslocked ) 
	...
	if fastenuf ~= vehicle.fastenuf then
		...
		if fastenuf then
			vehicle.track_snd = CreateSound( vehicle, "cpthazama/bwii/westernfrontier/VW_Recon_Eng.wav" )
